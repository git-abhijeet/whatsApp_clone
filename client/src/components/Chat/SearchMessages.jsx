...
function SearchMessages() {
  ...
  return (
    <div className="z-10 flex flex-col w-full max-h-screen border-l-4 border-conversation-border bg-conversation-panel-background">
      <div className="flex items-center h-16 gap-10 px-4 py-5 bg-panel-header-background text-primary-strong">
        <IoClose className="text-2xl cursor-pointer text-icon-lighter" onClick={() => dispatch({ type: reducerCases.SET_MESSAGE_SEARCH })} />
      ...
      <div className="h-full overflow-auto custom-scrollbar">
        <div className="flex flex-col items-center w-full">
          <div className="flex items-center w-full gap-3 px-5 h-14">
            <div className="flex items-center flex-grow gap-5 px-3 py-1 rounded-lg bg-panel-header-background">
              ...
              <div>
                <input type="text" placeholder="Search Messages" value={searchTerm} onChange={(e) => setSearchTerm(e.target.value)} className="w-full text-sm text-white bg-transparent focus:outline-none" />
              </div>
            </div>
          </div>
        ...
        <div className="flex flex-col justify-center h-full">
          {searchTerm.length > 0 && !searchedMessages.length && (<span className="flex justify-center w-full text-secondary"> No messages found</span>)}
          <div className="flex flex-col w-full h-full">
            {searchedMessages && searchedMessages.map((message) => (
